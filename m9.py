...
# Process Prompt Function (for LLM fallback)
...
def clean_data_with_ai(df, impute_method="auto", outlier_method="hybrid", z_threshold=3.0, iso_contamination=0.1, normalize=False):
    ...
    if stats_dict["missing_before"] > 0:
        ...
        if impute_method == "knn":
            ...
            cleaned_df[numeric_cols] = имputer.fit_transform(cleaned_df[numeric_cols])
...
# Define active_data here, before it's used
active_data = st.session_state.filtered_data if st.session_state.filtered_data is not None else st.session_state.data

# Sidebar Setup (Left)
with st.sidebar:
    st.markdown('<h2><span class="material-icons">settings</span> Control Panel</h2>', unsafe_allow_html=True)
    st.button("Toggle Sidebar", on_click=toggle_sidebar, help="Expand/Collapse Sidebar")
